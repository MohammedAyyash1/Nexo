...
const MAX_TEXT_LENGTH = 200; // حد Groq/Orpheus الفعلي للنص المدخل بميزة التحويل لصوت
...
// كشف بسيط للغة الفعلية للنص عبر نسبة الأحرف العربية مقابل اللاتينية
// null = نص بدون حروف عربية أو لاتينية (أرقام/رموز فقط) - ما فينا نحكم
...
export function TextToSpeechPage() {
  ...
  const handleGenerate = () => {
    ...
    if (!trimmed) { setError(t('الرجاء كتابة نص', 'Please enter some text')); return; }
    if (trimmed.length > MAX_TEXT_LENGTH) {
      setError(t(`النص طويل جدًا (الحد الأقصى ${MAX_TEXT_LENGTH} حرف)`, `Text too long (max ${MAX_TEXT_LENGTH} chars)`));
      return;
    }

    const detected = detectTextScript(trimmed);
    if (detected && detected !== voiceLang) {
      ...
      setError(
        t(
          `النص المكتوب يبدو بـ${detectedLabel}، لكن اللغة المختارة ${chosenLabel} — غيّر اللغة أو النص حتى ما يطلع الصوت مشوّهًا.`,
          `Your text looks like ${detectedLabel}, but the selected language is ${chosenLabel} — change the language or the text so the audio doesn't come out garbled.`
        )
      );
      return;
    }
    ...
    fetch(`${BASE}/tts/jobs`, {
      ...
      body: JSON.stringify({ text: trimmed, language: voiceLang, voice }),
    })
      .then(async (res) => {
      ...
      <div className="nexo-tool-page-header">
        ...
        <div>
          ...
          <p className="nexo-tool-page-desc">
            {t('اكتب نصًا واختر صوتًا، وسيقوم Nexo بتوليد ملف صوتي حقيقي بالعربية أو الإنجليزية. تأكد أن النص المكتوب بنفس اللغة المختارة.', "Write text and pick a voice — Nexo will generate a real audio file in Arabic or English. Make sure your text matches the selected language.")}
          </p>
        </div>
      </div>
